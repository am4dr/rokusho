...
class LibraryProviderCollection(
    providers: Set<LibraryProvider<*>>,
    ...
    override fun isAcceptable(descriptor: LibraryDescriptor): Boolean =
        providers.any { it.isAcceptable(descriptor) }

    override fun get(descriptor: LibraryDescriptor): Library<out Any>? =
        providers.find { isAcceptable(descriptor) }?.get(descriptor)
}
