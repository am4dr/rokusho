...
class DataObject(data: Map<String, String> = mapOf()) {
    ...
    private fun modify(modifier: (MutableMap<String, String>) -> Unit): DataObject =
        DataObject(data.toMutableMap().apply(modifier))


    fun diff(other: DataObject): Diff =
        Diff.of(this, other)
        ...
        companion object {
            fun of(left: DataObject, right: DataObject): Diff {
                ...
                return Diff(
                    lk.subtract(rk),
                    rk.subtract(lk),
                    lk.intersect(rk).filter { left[it] != right[it] }.toSet()
                )
            }
        }
