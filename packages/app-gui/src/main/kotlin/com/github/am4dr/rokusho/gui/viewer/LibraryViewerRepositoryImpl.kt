...
class LibraryViewerRepositoryImpl(private val viewerFactory: LibraryViewerFactory) : LibraryViewerRepository {
    ...
    private fun createLibraryViewAndCache(library: RokushoLibrary<*>): Node =
            viewerFactory(library).also { libraryViewCache[library] = SoftReference(it) }


    interface LibraryViewerFactory : (RokushoLibrary<*>) -> Node
    class RecordsViewersLibraryViewerFactory(private val recordsViewerFactories: List<RecordsViewerFactory>) : LibraryViewerFactory {
        override fun invoke(library: RokushoLibrary<*>): Node = createLibraryViewer(library.type, library)
